...
import { triggerClassify } from '../../services/populationSpaceService';

const PipelinePopulationClassifyPage = () => {
    usePageTitle('PopulationSpace: Classificador + SHAP');
    ...
    const run = async () => {
        ...
        try {
            ...
            const { task_id } = await triggerClassify(facilities, labelFeature, threshold ? parseFloat(threshold) : undefined);
            setTaskId(task_id);
        } catch (err) {
            alert(`Falha ao disparar classificador: ${err.response?.data?.error || err.message}`);
        }
    };

    React.useEffect(() => {
        if (isSuccess && taskId) {
            setTimeout(() => navigate(`/dashboards/population-classify/viewer?taskId=${taskId}`), 1200);
        }
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <header className="mb-8 text-center">
                <h1 className="text-3xl font-black text-gray-800 tracking-tight uppercase italic">Classificador + Explicação (SHAP)</h1>
                <p className="text-gray-500 mt-2 text-lg">RandomForest + SHAP sobre o PopulationSpace (BioSpace).</p>
            </header>

            <div className="max-w-4xl mx-auto space-y-8 pb-12">
                <InfoCard title="Como funciona">
                    <p className="text-sm leading-relaxed text-gray-600 font-medium">
                        Treina um RandomForest (biospace.prediction) para separar hospitais acima/abaixo da
                        mediana de um indicador, a partir do perfil de força de trabalho e demografia, e explica
                        cada previsão por SHAP (biospace.explainability) — quais variáveis mais pesaram.
                    </p>
                </InfoCard>
                ...
                <fieldset disabled={isPending} className="space-y-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                    <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                        <FiCpu className="text-blue-500" /> Hospitais
                    </h2>
                    <FacilitySearchPicker selected={selected} onChange={setSelected} suggestions={KNOWN_FACILITIES} />

                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1">Indicador (rótulo)</label>
                    ...
                    <button
                        onClick={run}
                        disabled={isPending || selected.length < 6}
                        className={`w-full relative py-3 rounded-xl font-black text-white uppercase text-sm tracking-widest transition-all overflow-hidden
                            ${isPending || selected.length < 6 ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 active:scale-[0.98]'}`}
                    >
                        {isPending && (
                        ...
                        <span className="relative z-10 flex items-center justify-center gap-2">
                            {isPending ? (
                                ...
                                `Classificar (${selected.length} hospitais, mín. 6)`
                            )}
                        </span>
                    </button>
                </fieldset>
...
export default PipelinePopulationClassifyPage;
