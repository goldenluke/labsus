...
import { triggerFactor } from '../../services/populationSpaceService';

const PipelinePopulationFactorPage = () => {
    usePageTitle('PopulationSpace: Fatores Latentes');
    ...
    const run = async () => {
        ...
        try {
            ...
            const { task_id } = await triggerFactor(facilities, nFactors);
            setTaskId(task_id);
        } catch (err) {
            alert(`Falha ao disparar análise fatorial: ${err.response?.data?.error || err.message}`);
        }
    };

    React.useEffect(() => {
        if (isSuccess && taskId) {
            setTimeout(() => navigate(`/dashboards/population-factor/viewer?taskId=${taskId}`), 1200);
        }
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <header className="mb-8 text-center">
                <h1 className="text-3xl font-black text-gray-800 tracking-tight uppercase italic">Fatores Latentes</h1>
                <p className="text-gray-500 mt-2 text-lg">Análise Fatorial — PopulationSpace (BioSpace).</p>
            </header>

            <div className="max-w-4xl mx-auto space-y-8 pb-12">
                <InfoCard title="Como funciona">
                    <p className="text-sm leading-relaxed text-gray-600 font-medium">
                        Extrai eixos de VARIAÇÃO COMPARTILHADA (biospace.latent, Análise Fatorial) entre as
                        12 Features de utilização hospitalar, força de trabalho e demografia — em vez de olhar
                        cada indicador isoladamente, encontra as poucas dimensões reais que explicam a maior
                        parte da diferença entre os estabelecimentos. A interpretação de cada fator é uma
                        hipótese exploratória, nunca um construto validado.
                    </p>
                </InfoCard>
                ...
                <fieldset disabled={isPending} className="space-y-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                    <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                        <FiLayers className="text-blue-500" /> Estabelecimentos (mín. 6)
                    </h2>
                    <FacilitySearchPicker selected={selected} onChange={setSelected} suggestions={KNOWN_FACILITIES} />

                    <div>
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1">Número de fatores</label>
                        <input
                            type="range" min="1" max="4" step="1" value={nFactors}
                            onChange={e => setNFactors(parseInt(e.target.value, 10))}
                            className="w-full h-2 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-blue-600"
                        />
                        <div className="text-center text-lg font-black text-blue-600">{nFactors}</div>
                    </div>
                    ...
                    >
                        ...
                        <span className="relative z-10 flex items-center justify-center gap-2">
                            {isPending ? (
                                ...
                                `Extrair fatores (${selected.length} estabelecimentos)`
                            )}
                        </span>
                    </button>
                </fieldset>
...
export default PipelinePopulationFactorPage;
