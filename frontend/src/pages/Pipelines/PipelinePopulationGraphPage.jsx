...
import { triggerGraph } from '../../services/populationSpaceService';
...
const PipelinePopulationGraphPage = () => {
    usePageTitle('PopulationSpace: Grafo de Similaridade');
    ...
    const run = async () => {
        ...
        try {
            ...
            const { task_id } = await triggerGraph(facilities, geometry, k, phenotypeK);
            setTaskId(task_id);
        } catch (err) {
            alert(`Falha ao disparar grafo de similaridade: ${err.response?.data?.error || err.message}`);
        }
    };

    React.useEffect(() => {
        if (isSuccess && taskId) {
            setTimeout(() => navigate(`/dashboards/population-graph/viewer?taskId=${taskId}`), 1200);
        }
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <header className="mb-8 text-center">
                <h1 className="text-3xl font-black text-gray-800 tracking-tight uppercase italic">Grafo de Similaridade</h1>
                <p className="text-gray-500 mt-2 text-lg">Rede de vizinhos mais próximos — PopulationSpace (BioSpace).</p>
            </header>

            <div className="max-w-4xl mx-auto space-y-8 pb-12">
                <InfoCard title="Como funciona">
                    <p className="text-sm leading-relaxed text-gray-600 font-medium">
                        Constrói uma rede real (biospace.graph) — nó por estabelecimento, aresta para cada um
                        dos K vizinhos mais próximos sob a geometria escolhida. Detecta comunidades (Louvain)
                        pela topologia do grafo e compara com o fenótipo (KMeans) encontrado no espaço de
                        representação — quando divergem, é um achado interessante sobre esse estabelecimento.
                    </p>
                </InfoCard>
                ...
                <fieldset disabled={isPending} className="space-y-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                    <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                        <FiShare2 className="text-blue-500" /> Estabelecimentos (mín. 6)
                    </h2>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1">Fenótipos (K, para o rótulo cruzado)</label>
                            <input
                    ...
                    >
                        ...
                        <span className="relative z-10 flex items-center justify-center gap-2">
                            {isPending ? (
                                ...
                                `Construir grafo (${selected.length} estabelecimentos)`
                            )}
                        </span>
                    </button>
                </fieldset>
...
export default PipelinePopulationGraphPage;
